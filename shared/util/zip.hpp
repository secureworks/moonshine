#ifndef MOONSHINE_SHARED_ZIP_HPP_
#define MOONSHINE_SHARED_ZIP_HPP_

/*
 * Based on https://github.com/tfussell/miniz-cpp
 */
...
class zip_file {
 public:
  ...
  explicit zip_file(const std::vector<unsigned char> &bytes) : zip_file() {
    load(bytes);
  }
  ...
  void reset() {
    switch (archive_->m_zip_mode) {
      case MZ_ZIP_MODE_READING:mz_zip_reader_end(archive_.get());
        break;
      case MZ_ZIP_MODE_WRITING:mz_zip_writer_finalize_archive(archive_.get());
        mz_zip_writer_end(archive_.get());
        break;
      case MZ_ZIP_MODE_WRITING_HAS_BEEN_FINALIZED:mz_zip_writer_end(archive_.get());
        break;
  ...
  void write(const std::string &arcname, const std::string &bytes) {
    if (archive_->m_zip_mode != MZ_ZIP_MODE_WRITING) {
      start_write();
    }
  ...
  void write(const zip_info &info, const std::string &bytes) {
 ...
 private:
  ...
  void remove_comment() {
    ...
    auto length = static_cast<uint16_t>(buffer_[position + 1]);
...
#endif //MOONSHINE_SHARED_ZIP_HPP_
