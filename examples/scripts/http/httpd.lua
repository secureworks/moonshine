...
--
-- https://github.com/keplerproject/xavante/blob/master/src/xavante/httpd.lua
--
-- Modified to support POST requests and other various small changes
-----------------------------------------------------------------------------
...
function _M.connection (skt)
    ...
    local chost, cport= skt:getpeername ()
    local req = {
        rawskt = skt,
        srv = srv,
        port = port,
...
function _M.read_content(req)
    local size = tonumber(req.headers["content-length"])
    req.content = ""
    local chunk = 1024
    while size > 0 do
        if (size < 1024) then chunk = size end
        local data, err = req.socket:receive(chunk)
        if (not data or err) then break end
        size = size - string.len(data)
        req.content = req.content .. data
    end
end
...
local function send_res_headers (res)
    ...
    --if package.loaded["httpd.cookies"] then
    --    local cookies = require "httpd.cookies"
...
function _M.send_response (req, res)
    ...
    if res.chunked or ((res.headers ["Content-Length"]) and string.lower (req.headers ["connection"] or "") == "keep-alive")
    then
