...
void dispatcher::push_received(const std::string& implant, const std::vector<shared::message>& messages)
{
  std::vector<std::shared_ptr<shared::message>> tmp;
  tmp.reserve(messages.size());
  for (const auto& result : messages) {
    tmp.push_back(std::make_shared<shared::message>(result.id, result.type, result.command, result.data, result.success));
  }

  push_received(implant, tmp);
}
